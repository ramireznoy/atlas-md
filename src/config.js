...
// Filenames to hide from the tree, override with ATLAS_MD_EXCLUDE=A.md,B.md
const raw = process.env.ATLAS_MD_EXCLUDE ?? 'README.md,CHANGELOG.md,LICENSE.md,CODE_OF_CONDUCT.md';
export const EXCLUDE_NAMES = new Set(raw.split(',').map(s => s.trim().toLowerCase()).filter(Boolean));

// Directory names to skip entirely, override with ATLAS_MD_EXCLUDE_DIRS=node_modules,dist
const rawDirs = process.env.ATLAS_MD_EXCLUDE_DIRS ?? 'node_modules';
